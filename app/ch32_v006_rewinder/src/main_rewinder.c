...
#define APP_VERSION_PATCH 0x00U
...
{
    /* speed 0 = stop, 1-255 = PWM duty proportional to PWM_PERIOD */
    uint32_t ccr;
    if (ch_1to4 < 1U || ch_1to4 > 4U) return;
    ccr = (speed == 0U) ? 0U : ((uint32_t)speed * (PWM_PERIOD + 1U) / 255U);
    g_motor_speed[ch_1to4 - 1U] = speed;
    switch (ch_1to4) {
    case 1: TIM2->CH1CVR = (uint16_t)ccr; break;
    case 2: TIM2->CH3CVR = (uint16_t)ccr; break;
    case 3: TIM1->CH2CVR = (uint16_t)ccr; break;
    case 4: TIM1->CH4CVR = (uint16_t)ccr; break;
    }
    argb_update_motors();
}
